...
function ajout_matiere($matiere,$coef){
...
function ajout_coefficient($coef){
	$coef_exists=mysql_query("select * from COEFFICIENT where num_coefficient='$coef'");
	if(($exists=mysql_num_rows($coef_exists))>0){
		$ligne=mysql_fetch_array($coef_exists);
		return $ligne[0];
	}else{
		if($coef=mysql_query("insert into COEFFICIENT(num_coefficient) values('$coef')")) return mysql_insert_id(); else return mysql_error();
	}			
}
...
function ajout_pp($nom,$prenom,$classe){
	$id_classe=mysql_query("insert into CLASSE(nom_classe) values('$classe')");
	$idclasse=mysql_insert_id();
	$idprofesseur=ajout_professeur($nom,$prenom);
	if($new_pp=mysql_query("update CLASSE set id_pp='$idprofesseur' where CLASSE.id_classe='$idclasse'")) return("insertion du professeur principal <b>".$nom." ".$prenom." reussie dans classe".$classe."!"); else return mysql_error();
...
function ajout_enseigne($nom,$prenom,$matiere,$classe){
	...
	$id_coefficient=ajout_coefficient($coef);
	...
	$enseigne_exist=mysql_query("select * from ENSEIGNE where id_classe='$idclasse[0]' and id_professeur='$idprofesseur' and id_matiere='$id_matiere[0]'");
        if(($exists=mysql_num_rows($enseigne_exist))>0){
        	...
        	return $nom." ".$prenom." ensegne deja la matiere ".$idmatiere[1]." a la classe ".$classe[1];
                ...
                if($enseigne=mysql_query("insert into ENSEIGNE(id_professeur,id_classe,id_matiere,id_coefficient) values('$idprofesseur','$idclasse[0]','$idmatiere[0]','$id_coefficient')")) print("insertion <b>enseignement</b> ok<br>"); else print(mysql_error());
	}
}
?>	
<HTML>	
