...
if(!isset($_POST['ajout']) || $_POST['ajout'] == "non"){
	?>
<SCRIPT>alert("PENSEZ A METTRE LE COEFFICIENT DE LA MATIERE DANS LE BLOC EN TETE DU DOCUMENT !!!");</SCRIPT>
<?php
...
if($_POST["classe"]=='CHOISIR') print("<script>alert('Vous n avez pas renseigne de classe !!');eval(document.location.href='commentaire.php');</script>");
	...
	$trimestre=$_POST['trimestre'];
		...
		if($inter!=NULL){
			...
			?>
			<table><tr><td bgcolor='pink'><u><font color='red'><center>coefficient de la mati&egrave;re DANS LE BULLETIN</u> : <select name='coef'>
			<?php
//			if($index_trim['id_matiere']==35 || $index_trim['id_matiere']==36) print("<option value=0.5>0.5");
//			else{ 
 				print("<option value=0.5>0.5");
				for($i=1;$i<=9;$i+=0.5){ 
					print("<option value=".$i);if($i==$index_trim['coef']) print(" selected >"); else print(">");
					print($i);
				}
//			}
			print("</select></td></tr></table>");
			print("TRIMESTRE : <input type='int' size=1 maxlength=1 name='trimestre' value='$trimestre'>");
			...
			print("<table><tr><th>Nø</th><th>NOM</th><th>Prenom</th>");
			$i=2;
			/*while($ligne_inter=mysql_fetch_array($inter)){
	...
	$ligne_matiere=mysql_fetch_array($all_matiere);
	$all_classe=mysql_query("select * from ELEVE where id_classe='$classe' ORDER BY ELEVE.nom_eleve ASC");
	print("<h1>Appreciation de la classe de ".$nom_classe[1]." en ".$ligne_matiere[1]." pour le trimestre ".$trimestre." coefficient ".$coef."</h1>");
	$i=1;
	...
	while($ligne_classe=mysql_fetch_array($all_classe)){
		...
		if(!isset($ligne_com["appreciation_generale"]) && !isset($ligne_com["conseil_pour_progresser"])){
			...
			//if($comment != "" || $conseil != ""){
				...
				if(mysql_query("insert into MOYENNE_TRIMESTRE(id_eleve,id_matiere,id_trimestre,appreciation_generale,conseil_pour_progresser,moyenne,coef,moyenne_classe) values(".$ligne_classe[0].",".$matiere.",".$trimestre.",'".utf8_decode($comment)."','".utf8_decode($conseil)."','".$moyenne."','".$coef."','".$moy_classe."')"));
			...
			$comment=$_POST['comment'.$i];print("<script>alert($comment);</script>");
			...
			if(mysql_query("update MOYENNE_TRIMESTRE set MOYENNE_TRIMESTRE.appreciation_generale='".utf8_decode($comment)."', conseil_pour_progresser='".utf8_decode($conseil)."', moyenne='$moyenne', coef='$coef', moyenne_classe='$moy_classe' where id_eleve='$ligne_classe[0]' and id_matiere='$matiere' and id_trimestre='$trimestre'"))
			print("<tr><td>".$i."</td><td>".$ligne_classe[1]."</td><td>".$moyenne."</td><td>".$comment."</td><td>".$conseil."</td></tr>");
